...
module Arrays = struct
    ...
    let find_index arr elem =
        let found_index = ref None in
        let current_index = ref 0 in
        let arr_len = Array.length arr in
        while (!current_index < arr_len) && (!found_index = None) do
          if arr.(!current_index) = elem
          then found_index := Some !current_index;
          incr current_index
        done;
        !found_index

    let find_index_exn arr elem =
        match find_index arr elem with
        | Some idx -> idx
        | None -> failwith "Element not found in array!"
end
