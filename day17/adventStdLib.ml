...
module Arrays = struct
    ...
    let insert_at arr idx value =
        let array_length = Array.length arr in
        match idx with
        | 0 -> Array.append [|value|] arr
        | x when x = array_length -> Array.append arr [|value|]
        | x when x > array_length -> failwith (Printf.sprintf "Index out of range: %d" x)
        | _ -> 
            let prefix = ArrayLabels.sub arr ~pos:0 ~len:idx in
            let suffix = ArrayLabels.sub arr ~pos:idx ~len:(array_length - idx) in
            Array.concat [prefix; [|value|]; suffix]

    let index_of x arr : int option =
        let array_length = Array.length arr in
        if array_length = 0 then None
        else
            let current_index = ref 0 in
            let found_index = ref None in
            while (!current_index < array_length) && (!found_index = None) do
                if (arr.(!current_index) = x) then 
    ...
    let to_string (stringifier: 'a -> string) arr : string =
        arr
        |> Array.to_list 
